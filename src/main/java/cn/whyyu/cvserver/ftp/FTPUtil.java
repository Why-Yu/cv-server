...
public class FTPUtil {
    ...
    public DownloadStatus download(String remote, String local) throws IOException {
        ...
        try {
            // 检查远程文件是否存在
            FTPFile[] files = ftpClient.listFiles(new String(remote.getBytes(StandardCharsets.UTF_8), StandardCharsets.ISO_8859_1));
            if (files.length != 1) {
                log.info("远程文件不存在");
            ...
            if (f.exists()) {
                ...
                InputStream in = ftpClient.retrieveFileStream(new String(remote.getBytes(StandardCharsets.UTF_8), StandardCharsets.ISO_8859_1));
     ...
     * 创建远程目录
     *
     * @param pathName 以linux格式，直接在根目录上创建
     * @return
     * @throws IOException
     */
    ...
    public UploadStatus upload(String local, String remote) {
        ...
        try {
            ftpClient.changeWorkingDirectory(ftpPoolService.getFtpConfig().getWorkingDirectory());
            // 设置PassiveMode传输
            ftpClient.enterLocalPassiveMode();
            // 设置以二进制流的方式传输
            ftpClient.setFileType(FTP.BINARY_FILE_TYPE);
            ...
            // 检查远程是否存在文件
            FTPFile[] files = ftpClient.listFiles(new String(remoteFileName.getBytes(StandardCharsets.UTF_8), StandardCharsets.ISO_8859_1));
            if (files.length == 1) {
     ...
     * 递归创建远程服务器目录
     *
     * @param remote    远程服务器文件绝对路径例如image/raw/
    ...
    public UploadStatus createDirecroty(String remote, FTPClient ftpClient) throws IOException {
        ...
        if (!directory.equalsIgnoreCase("/") && !ftpClient.changeWorkingDirectory(new String(directory.getBytes(StandardCharsets.UTF_8), StandardCharsets.ISO_8859_1))) {
     ...
     * @param local 图片文件路径
     */
    public void createLocalDirectory(String local) {
        String directory = local.substring(0, local.lastIndexOf("/") + 1);
        File targetPath = new File(directory);
        targetPath.mkdirs();
    }
